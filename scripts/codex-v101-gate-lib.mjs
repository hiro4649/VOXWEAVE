...
export function buildV101SelfTestRegistrationReport(input = {}) {
  const reasons = [];
  const manifestFile = process.env.CODEX_HARNESS_MODE === 'target' && fs.existsSync('docs/process/CODEX_HARNESS_MANIFEST.json') ? 'docs/process/CODEX_HARNESS_MANIFEST.json' : 'CODEX_SOURCE_HARNESS_MANIFEST.json';
  ...
  if (!readText(manifestFile)?.includes('codex-v101-self-test.mjs')) reasons.push('v101_self_test_missing');
