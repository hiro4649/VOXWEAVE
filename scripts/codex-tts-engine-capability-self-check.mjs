...
assert.equal(moss.real_tts_ready, false);
...
assert.equal(mossRealtime.real_tts_ready, false);
...
const realTtsReady = validateTtsEngineCapabilityProfile(mockTtsProfile({
  real_tts_ready: true,
}));
assert.equal(realTtsReady.blocked, true);
assert.equal(
  realTtsReady.reason_codes.includes("real_tts_ready_claim_prohibited_for_candidate_slice"),
  true,
);
...
assert.equal(unsafe.unsafe_fields_present.length >= 15, true);
assert.equal(unsafe.unsafe_detection_reason_counts.endpoint_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.api_key_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.api_key_dash_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.token_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.secret_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.authorization_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.bearer_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.model_path_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.dataset_path_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.raw_payload_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.raw_logs_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.reference_voice_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.prompt_audio_detected > 0, true);
assert.equal(unsafe.unsafe_detection_reason_counts.generated_audio_ref_detected > 0, true);
...
const summary = buildTtsEngineCapabilitySafeSummary([
  ...
  mockTtsProfile({ real_tts_ready: true }),
  mossTtsProfile({
    endpoint: "https://tts-engine.invalid",
    api_key: "api-key",
    token: "token",
...
assert.equal(summary.engine_count, 7);
...
assert.equal(summary.real_tts_ready_count, 0);
...
const unsafeSummary = buildTtsEngineCapabilityUnsafeDetectionSafeSummary([unsafe]);
assert.equal(unsafeSummary.safe_summary_only, true);
assert.equal(unsafeSummary.unsafe_count, 1);
assert.equal(unsafeSummary.reason_counts.unknown_unsafe_field_detected > 0, true);

const serialized = JSON.stringify({ summary, unsafeSummary });
