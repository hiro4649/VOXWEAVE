...
function statusForHead(values, headSha) {
  ...
  return values.includes(headSha) ? 'pass' : 'fail';
}

function ownerFor({ liveStatus, artifactStatus, diffStatus, reasonCodes }) {
  const hasStaleReason = reasonCodes.includes('stale_confirmation_detected') || reasonCodes.includes('stale_evidence');
  if (!hasStaleReason) return 'not_applicable';
  if (liveStatus === 'pass' && (artifactStatus !== 'pass' || diffStatus === 'mismatch')) {
...
export function classifyStaleAuditFreshness(input = {}) {
  ...
  const reasonCodes = Array.isArray(input.staleAuditReasonCodes) ? input.staleAuditReasonCodes.map(String) : [];
  const prBodyLiveFetchStatus = statusForHead(liveValues, currentHead);
  const githubEventPayloadFreshnessStatus = eventValues.length && statusForHead(eventValues, currentHead) === 'pass'
    ? 'pass'
    : eventValues.length ? 'stale_or_unknown' : 'unknown';
  const safeArtifactHeadMatchStatus = artifactValues.length ? statusForHead(artifactValues, currentHead) : 'unknown';
  const safeArtifactFreshnessStatus = safeArtifactHeadMatchStatus === 'pass' ? 'pass' : 'stale_or_unknown';
  const eventPayloadVsLivePrBodyDiffStatus = eventValues.length && liveValues.length && eventValues.join(',') !== liveValues.join(',')
    ? 'mismatch'
    : 'not_detected';
  ...
  const owner = ownerFor({
    ...
    diffStatus: eventPayloadVsLivePrBodyDiffStatus,
    reasonCodes,
  });
  const staleAuditInputStatus = owner === 'not_applicable'
    ? 'pass'
    : owner === 'pr_body'
      ? 'pr_body_blocked'
  ...
  return {
    ...
    staleConfirmationOwnerStatus: owner,
    staleEvidenceOwnerStatus: owner,
    safeNextAction: 'Investigate stale audit input freshness without changing merge rules.',
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  const result = classifyStaleAuditFreshness({
    ...
    staleAuditReasonCodes: String(process.env.CODEX_STALE_AUDIT_REASON_CODES || '').split(',').map((code) => code.trim()).filter(Boolean),
  });
