...
import { buildDictionarySafeSummary, getNormalizationDictionaryEntries } from "../src/common/textNormalizationDictionary.js";
...
check("v1.1.3 explicit scope fixture pass", () => {
  const status = classifyNormalizationEvidencePrecedence({
    chatgptProScopeGrantRecorded: true,
    selfCheckCarryForward: true,
    qgStatus: "success"
  });
  assert.equal(status, "candidate_branch_non_runtime_evidence_only");
});

check("evidence precedence fixture blocks missing scope", () => {
  assert.equal(classifyNormalizationEvidencePrecedence({ selfCheckCarryForward: true, qgStatus: "success" }), "blocked_by_missing_scope_grant");
});

check("product verification fixture blocks QG failure", () => {
  assert.equal(classifyNormalizationEvidencePrecedence({ chatgptProScopeGrantRecorded: true, selfCheckCarryForward: true, qgStatus: "failure" }), "blocked_by_qg");
});
...
for (const field of ["runtimeConnected", "activeQGConnected", "workflowChanged", "packageChanged"]) {
  check(`${field} is blocked`, () => {
    ...
    assert.notEqual(status, "candidate_branch_non_runtime_evidence_only");
  });
}
...
check("no adapter import behavior", () => {
  const result = normalizeSafeText("adapter");
  assert.equal(result.mainEvidenceStatus, "no");
});
...
check("no model download behavior", () => {
  ...
  assert.equal(result.candidateBranchEvidenceStatus, "yes");
});
...
check("candidate branch evidence status yes", () => {
  assert.equal(normalizeSafeText("candidate").candidateBranchEvidenceStatus, "yes");
});
...
check("main evidence status no until merged", () => {
  assert.equal(normalizeSafeText("main").mainEvidenceStatus, "no");
});
...
const report = {
  ...
  normalization_main_reflection_execution_candidate: true,
  chatgpt_pro_scope_grant_recorded: true,
  dictionary_carry_forward: true,
  self_check_carry_forward: true,
  v113_compatibility: true,
