...
export function normalizeSafeText(input, options = {}) {
  ...
  return {
    ...
    replacementCandidateEvidenceStatus: "yes",
...
export function classifyNormalizationEvidencePrecedence(input = {}) {
  ...
  if (input.serverConnected) return "blocked_by_server_connection";
  if (input.adapterConnected) return "blocked_by_adapter_connection";
  ...
  if (!input.postDiagnosticReevaluationRecorded) return "blocked_by_missing_post_diagnostic_re_evaluation";
  if (!input.selfCheckCarryForward) return "blocked_by_missing_self_check_carry_forward";
  return "main_based_non_runtime_replacement_candidate_only";
}
