...
function buildLegacyUnsafeFindings(manifest) {
  return findUnsafeFields(manifest).map((field) => {
    const reasonByField = {
      "api-key": "api_key_dash_detected",
      api_call_payload: "unknown_unsafe_field_detected",
      bearer: "bearer_detected",
      dataset_path: "dataset_path_detected",
      download_url: "url_detected",
      endpoint: "endpoint_detected",
      endpoint_url: "endpoint_detected",
      model_download_url: "url_detected",
      model_path: "model_path_detected",
      package_change_note: "unknown_unsafe_field_detected",
      private_path: "private_path_detected",
      raw_benchmark_logs: "raw_logs_detected",
      raw_logs: "raw_logs_detected",
      raw_manifest: "unknown_unsafe_field_detected",
      raw_payload: "raw_payload_detected",
      workflow_path: "unknown_unsafe_field_detected",
    };
    return {
      category: "field",
      reason_code: reasonByField[field] ?? `${field}_detected`,
      count: 1,
    };
  });
}

export function detectUnsafeTtsBenchmarkManifestFields(manifest = {}) {
  return [
    ...detectUnsafeFields(manifest),
    ...buildLegacyUnsafeFindings(manifest),
  ];
}

export function hasUnsafeTtsBenchmarkManifestFields(manifest = {}) {
  return hasUnsafeFields(manifest) || buildLegacyUnsafeFindings(manifest).length > 0;
}

export function buildTtsBenchmarkUnsafeDetectionSafeSummary(manifests = []) {
  const items = Array.isArray(manifests) ? manifests : [];
  return buildUnsafeFieldDetectionSafeSummary(
    items.map((manifest) => detectUnsafeTtsBenchmarkManifestFields(manifest)),
  );
}
